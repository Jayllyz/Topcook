    ...
    if(isset($_POST['submit'])){
        ...
        if(isset($_FILES['image']) && !empty($_FILES['image']['name'])){
            ...
            if(!in_array($_FILES['image']['type'], $acceptable)){
                ...
                header('location: ../inscription.php?message=Type de fichier incorrect.&type=danger');
                exit;
            }
            ...
            if ($reponse) {
                header('location: ../inscription.php?message=Cet email est déja utilisé !&type=danger');
                exit;
            }
            // Fin vérif email

            if(!empty($_POST['pseudo']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['conf_password']) && !empty($_POST['birth'])){
                if($_POST['password'] == $_POST['conf_password']){
                    $req = $db->prepare('INSERT INTO USER (pseudo,email,password,date_birth,image) VALUES (:pseudo,:email,:password,:date_birth,:image)');
                    $email = $_POST['email'];
                    $pseudo = $_POST['pseudo'];
