...
if (isset($_POST["submit"])) {
  ...
  if (strlen($_POST["password"]) < 6 || strlen($_POST["password"]) > 15) {
    header(
      "location: inscription.php?message=Password invalide ! Il doit etre compris entre 6 et 15 caractères !"
    );
    exit();
  }
  ...
  if (
    ...
    if ($_POST["password"] == $_POST["conf_password"]) {
      ...
      header(
        "location: inscription.php?message=Les mots de passes ne sont pas identiques !"
      );
      exit();
    }
  ...
  header(
    "location:inscription.php?message=Les champs ne sont pas tous remplis !"
  );
  exit();
}
